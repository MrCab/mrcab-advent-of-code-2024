...
class MultiplicationSolver :
  ...
  def __init__( self ):
    ...
    self.searchRegex = re.compile( '^.*?(?P<command>mul\((?P<num1>\d{1,3}),(?P<num2>\d{1,3})\)|do\(\)|don\'t\(\))(?P<theRest>.*)' )
    ...
    for line in self.linesOfCode :
       ...
       while m != None :
         ...
         if command.startswith( "don't" ) :
           self.actuallyDoIt = False
         elif command.startswith( "do()" ) :
           self.actuallyDoIt = True
         elif command.startswith( "mul" ) and self.actuallyDoIt :
